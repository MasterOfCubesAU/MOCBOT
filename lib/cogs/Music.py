...
class Music(commands.Cog):
    ...
    async def ensure_voice(self, interaction):
        ...
        if not v_client:
            if not should_connect:
                await self.send_message(interaction, 'MOCBOT is not connected to a voice channel.', True)
                return False

            permissions = interaction.user.voice.channel.permissions_for(interaction.guild.me)

            if not permissions.connect or not permissions.speak:  # Check user limit too?
                await self.send_message(interaction, 'Please provide MOCBOT with the CONNECT and SPEAK permissions.', True)
                return False
            await interaction.user.voice.channel.connect(cls=LavalinkVoiceClient)
        else:
            if v_client.channel.id != interaction.user.voice.channel.id:
                await self.send_message(interaction, 'You need to be in my voice channel to execute that command.', True)
                return False
    ...
    async def skip(self, interaction: discord.Interaction, position: typing.Optional[int] = 1):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The skip command requires media to be playing first.", True)
        if position < 1 or (position > len(player.queue) and not player.fetch("autoplay") and player.loop == player.LOOP_NONE):
        ...
        await player.skip()
        await self.send_message(interaction, f"Successfully skipped the track [{skipped.title}]({skipped.uri}).")
    ...
    async def seek(self, interaction: discord.Interaction, time: str):
        if (time := Music.convert_to_seconds(time)) is None:
            return await self.send_message(interaction, f"Please provide the time to seek in a suitable format.\nExamples: `10 | 1:10 | 1:10:10`")

        player = self.bot.lavalink.player_manager.get(interaction.guild.id)
        if player is None or player.current is None:
            return await self.send_message(interaction, "The seek command requires media to be playing first.", True)
    ...
    async def loop(self, interaction: discord.Interaction, type: Literal["Off", "Song", "Queue"]):
        player = self.bot.lavalink.player_manager.get(interaction.guild.id)
        if player is None or player.current is None:
            return await self.send_message(interaction, f"The loop command requires media to be playing first.", True)
        match type:
            case "Off":
                player.set_loop(0)
            ...
            case "Song":
                player.set_loop(1)
                await self.send_message(interaction, f"Song looping has been enabled.")
            case "Queue":
                player.set_loop(2)
    ...
    async def stop(self, interaction: discord.Interaction):
        ...
        if not interaction.user.voice or (player.is_connected and interaction.user.voice.channel.id != int(player.channel_id)):
            ...
            await self.send_message(interaction, "You must be in the same channel as MOCBOT to use this command.", True)
    ...
    async def pause(self, interaction: discord.Interaction):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The pause command requires media to be playing first.", True)
        if player.paused:
            return await self.send_message(interaction, "The music queue is already paused. Use the resume command to resume the music.", True)
    ...
    async def resume(self, interaction: discord.Interaction):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The resume command requires media to be playing first.", True)
    ...
    async def shuffle(self, interaction: discord.Interaction):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The shuffle command requires media to be playing first.", True)
    ...
    async def remove(self, interaction: discord.Interaction, start: int, end: typing.Optional[int]):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The remove command requires media to be playing first.", True)
        if start < 1 or start > len(player.queue):
            return await self.send_message(interaction, f"Please enter a valid track number to remove. {self.queue_length_msg(player)}", True)

        if end is None:
            removedTrack = player.queue.pop(start - 1)
            return await self.send_message(interaction, f"Successfully removed track [{removedTrack.title}]({removedTrack.uri}).")
        else:
            if end < start or end > len(player.queue):
                await self.send_message(interaction, f"Please enter a valid range to remove. {self.queue_length_msg(player)}", True)
            player.queue = player.queue[:start - 1] + player.queue[end:]
            await self.send_message(interaction, f"Successfully removed **{end - start + 1} track{'' if end - start + 1 == 1 else 's'}** from the queue.")
    ...
    async def move(self, interaction: discord.Interaction, source: int, destination: int):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The move command requires media to be playing first.", True)
        if len(player.queue) < 2:
            return await self.send_message(interaction, "There are less than two songs in the queue, so there are no songs to move.", True)
        if source == destination: 
            return await self.send_message(interaction, "You can only move songs to a different position in the queue.", True)
        if source < 1 or source > len(player.queue) or destination < 1 or destination > len(player.queue):
            return await self.send_message(interaction, f"Please enter valid positions in the queue. {self.queue_length_msg()}", True)

        player.queue.insert(destination - 1, track:=player.queue.pop(source - 1))
        await self.send_message(interaction, f"Successfully moved track [{track.title}]({track.uri}) to position **{destination}** in the queue.")
    ...
    async def now_playing(self, interaction: discord.Interaction):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The now playing command requires media to be playing first.", True)
    ...
    async def jump(self, interaction: discord.Interaction, position: int):
        player = self.bot.lavalink.player_manager.get(interaction.guild.id)
        if player is None or player.current is None:
            return await self.send_message(interaction, "The jump command requires media to be playing first.", True)
        ...
        await player.play()
        await self.send_message(interaction, f"Successfully jumped to track [{player.current.title}]({player.current.uri}).")
    ...
    async def autoplay(self, interaction: discord.Interaction, type: Literal["Off", "On"]):
        ...
        if player is None or player.current is None:
            return await self.send_message(interaction, "The autoplay command requires media to be playing first.", True)
        match type:
            case "Off":
                player.store("autoplay", False)
                await self.send_message(interaction, f"Autoplaying has been disabled for the queue.")
            case "On":
    ...
    async def lyrics(self, interaction: discord.Interaction, query: typing.Optional[str]):
        ...
        if not lyrics and query is None:
            return await self.send_message(interaction, f"No lyrics were found for **{player.current.title}**", True)
        elif not lyrics and query is not None:
            return await self.send_message(interaction, f"No lyrics were found for **{query}**. Try searching again using the format: `(Song Name) - (Artist)`.", True)
    ...
    async def rewind(self, interaction: discord.Interaction, time: typing.Optional[str]):
        if (converted_time := Music.convert_to_seconds(time)) == -1:
            ...
            return await self.send_message(interaction, "Please provide the time to rewind in a suitable format.\nExamples: `10 | 1:10 | 1:10:10`", True)

        player = self.bot.lavalink.player_manager.get(interaction.guild.id)
        if player is None or player.current is None:
            return await self.send_message(interaction, "The rewind command requires media to be playing first.", True)
        if not player.current.is_seekable:
    ...
    async def fastforward(self, interaction: discord.Interaction, time: typing.Optional[str]):
        ...
        player = self.bot.lavalink.player_manager.get(interaction.guild.id)
        if player is None or player.current is None:
            return await self.send_message(interaction, "The fast forward command requires media to be playing first.", True)
        if not player.current.is_seekable:
